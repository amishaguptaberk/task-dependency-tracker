...
Earliest start dates are calculated but not shown in the UI. The spec says "calculate the earliest possible start date", where the critical path and dependency graph requirements say "show" and "visualize". So the forward pass in `computeSchedule` calculates it, the `'D earliest start'` assertion in `lib/scheduling.ts` verifies it (`schedule.earliestStart.D === 6`), and `GET /api/todos/schedule` serves it. Rendering it in the list would be scope the spec didn't ask for.
